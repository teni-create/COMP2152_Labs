...
def binary_search_iterative(nums, target):
    left = 0
    right = len(nums) - 1 #initialize right pointer to len(nums) - 1
    #While left <= right:
    while left <= right:
    ...
    #   - If nums[mid] == target, return mid
        if nums[mid] == target:
            return mid
    #   - If target < nums[mid], move right pointer: right = mid -1
        elif target < nums[mid]:
            right = mid - 1
    #   - If target > nums[mid], move left pointer: left = mid + 1
        if target > nums[mid]:
                left = mid + 1
